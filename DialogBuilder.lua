...
local function getActivePath()
    if ENV.isGrowLauncher then return RTTEX_PATH_GL end
    return RTTEX_PATH
end
...
function ihkaz.importrttex(prefix)
    ...
    if not checkRttexPermission() then
        logger("Error: No write permission to RTTEX path: " .. getActivePath(), true)
    ...
    for _, v in ipairs(items) do
        if not v.url or not v.name then
            ...
            if not data or data == "" then
                ...
                if v.b64 or ENV.isGrowLauncher then
                    data = b64decode(data)
                end
                os.execute("mkdir -p " .. activePath)
                local f = io.open(activePath .. v.name, "wb")
                if f then
                    f:write(data)
                    f:close()
                    logger("Success Import RTTEX: " .. v.name .. " (" .. #data .. " bytes)")
                else
...
return ihkaz        logger("Tip: Growtopia harus sudah pernah dibuka setidaknya sekali", false)
        return false
    end
